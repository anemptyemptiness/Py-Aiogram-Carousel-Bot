...
async def report_money_in_range(date_from: str, date_to: str, msg: Message):
    ...
    places = {
        "Новая Рига": sum([row[0].count("Новая Рига") for row in rows]),
        "Белая Дача": sum([row[0].count("Белая Дача") for row in rows]),
        "Внуково": sum([row[0].count("Внуково") for row in rows]),
        "Саларис": sum([row[0].count("Саларис") for row in rows]),
    }
...
@router_adm.message(Command(commands="stats"), IsAdminFilter(config.admins), StateFilter(default_state))
